//! `ogt report`: turn the tracking JSONL into a falsifiable savings
...
//! - `reads_fold` only catches a re-read that goes back through ogt's
//!   own argv. A re-read that bypasses ogt entirely (a plain shell
//!   `cat` of a fold file, run outside ogt) is invisible here, so the
