//! Entry point for `sigfold hook`, wired from `cli::dispatch`. No flags:
...
mod tests {
    ...
    fn rewrite_target_prints_the_hookspecificoutput_envelope() {
        ...
        assert!(out.contains(r#""command":"sigfold grep foo src/""#));
        assert!(out.contains(r#""description":"search""#));
        assert!(
            !out.contains("permissionDecision"),
            "sigfold must never assert a permission decision"
        );
    }
    ...
    fn already_wrapped_command_is_idempotent() {
        let json = r#"{"tool_name":"Bash","tool_input":{"command":"sigfold grep foo src/"}}"#;
        let (code, out) = run_on(json);
        assert_eq!(code, 0);
        assert!(out.is_empty());
    }
    ...
    fn preserves_extra_tool_input_fields_verbatim() {
        ...
        assert!(out.contains(r#""command":"sigfold cat file.txt""#));
    }
}
