//! `ogt hook`: the PreToolUse hook Claude Code invokes before running a
...
//!
//! `permissionDecision` is never set: ogt makes no policy judgment,
...
//! `allow` from a tool with no permission-rule engine is how ogt's
