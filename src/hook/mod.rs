//! `sigfold hook`: the PreToolUse hook Claude Code invokes before running a
...
//!
//! `permissionDecision` is never set: sigfold makes no policy judgment,
...
//! `allow` from a tool with no permission-rule engine is how sigfold's
