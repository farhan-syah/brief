...
mod tests {
    ...
    fn splice_replaces_only_the_command_value() {
        ...
        let updated = splice_command(tool_input, "ogt grep foo").expect("splice must succeed");
        assert_eq!(
            updated,
            r#"{"command":"ogt grep foo","description":"a desc, with a comma","timeout":5}"#
        );
    }
    ...
    fn splice_preserves_field_order_and_unknown_fields() {
        ...
        let updated = splice_command(tool_input, "ogt grep foo").unwrap();
        assert_eq!(
            updated,
            r#"{"description":"d","command":"ogt grep foo","weird_field":{"nested":[1,2,3]}}"#
        );
    }

    #[test]
    fn build_output_shape() {
        let out = build_output(r#"{"command":"ogt grep foo"}"#);
        assert_eq!(
            out,
            "{\"hookSpecificOutput\":{\"hookEventName\":\"PreToolUse\",\"updatedInput\":{\"command\":\"ogt grep foo\"}}}\n"
        );
    }
}
