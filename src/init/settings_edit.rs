...
const HOOK_COMMAND: &str = "ogt hook";
...
/// `ogt hook` is already present anywhere in `settings`.
#[cfg(test)]
...
/// Remove exactly the hook-entry object whose command is `ogt hook`
...
/// hook entry whose command is exactly `ogt hook`.
...
mod tests {
    ...
    fn no_hooks_key_installs_fresh_structure() {
        ...
        assert!(new.contains("\"command\": \"ogt hook\""));
    ...
    fn already_present_is_reported_and_nothing_changes() {
        let settings = r#"{"hooks":{"PreToolUse":[{"matcher":"Bash","hooks":[{"type":"command","command":"ogt hook"}]}]}}"#;
    ...
    fn present_among_other_matcher_groups_is_still_detected() {
        let settings = r#"{"hooks":{"PreToolUse":[{"matcher":"Edit","hooks":[{"type":"command","command":"lint.sh"}]},{"matcher":"Bash","hooks":[{"type":"command","command":"ogt hook"}]}]}}"#;
    ...
    fn removing_the_only_entry_drops_the_empty_matcher_group() {
        ...
        assert!(!removed.contains("ogt hook"));
    ...
    #[test]
    fn remove_deletes_only_the_ogt_entry() {
        let settings = r#"{"hooks":{"PreToolUse":[{"matcher":"Edit","hooks":[{"type":"command","command":"lint.sh"}]},{"matcher":"Bash","hooks":[{"type":"command","command":"ogt hook"}]}]}}"#;
        ...
        assert!(!new.contains("ogt hook"));
        assert!(!find_hook_entry(&new).unwrap());
    }

    #[test]
    fn remove_leaves_other_hooks_in_the_same_matcher_group_untouched() {
        let settings = r#"{"hooks":{"PreToolUse":[{"matcher":"Bash","hooks":[{"type":"command","command":"lint.sh"},{"type":"command","command":"ogt hook"}]}]}}"#;
        let RemoveOutcome::Removed(new) = remove_hook_entry(settings).unwrap() else {
            panic!("expected Removed");
        };
        assert!(new.contains("lint.sh"));
    ...
    fn comma_inside_a_string_value_never_splits_a_field() {
        let settings = r#"{"hooks":{"PreToolUse":[{"matcher":"Bash","hooks":[{"type":"command","command":"ogt hook","description":"a, b, c"}]}]}}"#;
        assert!(find_hook_entry(settings).unwrap());
        let RemoveOutcome::Removed(new) = remove_hook_entry(settings).unwrap() else {
            panic!("expected Removed");
        };
