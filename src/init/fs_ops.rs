//! Filesystem side of `ogt init`: locating settings.json under an
...
/// Run `ogt init`. `home_dir` is injected so tests can point this at a
...
pub(crate) fn run_with(
    ...
    let Some(home_dir) = home_dir else {
        let _ = writeln!(err, "ogt init: could not determine the home directory");
        return 1;
    };
...
fn run_install(
    ...
    match insert_hook_entry(contents) {
        Ok(InsertOutcome::AlreadyPresent) => {
            let _ = writeln!(
                out,
                "ogt hook already installed in {}; nothing to do",
                settings_path.display()
            );
            0
        }
        Ok(InsertOutcome::Inserted(new_contents)) => {
            if dry_run {
                let _ = writeln!(
                    out,
                    "Would install the ogt PreToolUse hook in {}:\n",
                    settings_path.display()
                );
            ...
            match write_settings(settings_path, contents, &new_contents, existed) {
                Ok(()) => {
                    let _ = writeln!(
                        out,
                        "Installed the ogt PreToolUse hook in {}",
                        settings_path.display()
                    );
                    0
                }
                Err(e) => {
                    let _ = writeln!(err, "ogt init: {e}");
                    1
                }
            }
        }
        Err(_unrecognized) => {
            let _ = writeln!(
                err,
                "ogt init: {} isn't a shape I confidently recognize, so I won't guess at editing it.\n\
...
fn run_uninstall(
    ...
    match remove_hook_entry(contents) {
        Ok(RemoveOutcome::NotPresent) => {
            let _ = writeln!(
                out,
                "ogt hook not installed in {}; nothing to do",
                settings_path.display()
            );
            0
        }
        Ok(RemoveOutcome::Removed(new_contents)) => {
            if dry_run {
                let _ = writeln!(
                    out,
                    "Would remove the ogt PreToolUse hook from {}:\n",
                    settings_path.display()
                );
                let _ = writeln!(out, "{new_contents}");
                return 0;
            ...
            match write_settings(settings_path, contents, &new_contents, existed) {
                Ok(()) => {
                    let _ = writeln!(
                        out,
                        "Removed the ogt PreToolUse hook from {}",
                        settings_path.display()
                    );
                    0
                }
        ...
        Err(_unrecognized) => {
            let _ = writeln!(
                err,
                "ogt init --uninstall: {} isn't a shape I confidently recognize, \
                 so I won't guess at editing it. Remove the ogt hook entry from \
...
fn write_settings(
    ...
    let tmp_path = dir.join(format!(".ogt-settings-{}.tmp", std::process::id()));
...
mod tests {
    ...
    fn install_creates_settings_json_when_missing() {
        ...
        assert!(written.contains("ogt hook"));
    ...
    fn unrecognized_shape_refuses_to_write_and_prints_the_block() {
        ...
        assert!(printed.contains("ogt hook"));
