//! Append-only invocation tracking: one audit row per `ogt` run,
...
//!
//! `reads_fold` is the one re-read signal ogt can observe honestly:
//! whether this invocation's own argv resolves into the fold directory.
//! A read that bypasses ogt entirely (a shell `cat` on a fold file run
//! outside ogt) is not observable here, so `reads_fold` undercounts
...
//! fully inherited stdio, which ogt never observes and therefore cannot
//! measure — except that a non-target invocation whose own argv reads back
//! a fold file (the `ogt tail ...` the fold hint prescribes) still
...
//! denominator for "output ogt handled", not "output the machine
