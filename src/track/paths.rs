...
/// Resolve the tracking file path: `SIGFOLD_TRACK_FILE` env var, then
/// `cfg.path`, then the default `dirs::data_local_dir()/sigfold/tracking.jsonl`.
...
pub(crate) fn resolve_track_path_with(
    ...
    if let Some(path) = lookup("SIGFOLD_TRACK_FILE") {
        return Some(PathBuf::from(path));
    }
    if let Some(ref path) = cfg.path {
        return Some(path.clone());
    }
    dirs::data_local_dir().map(|d| d.join("sigfold").join("tracking.jsonl"))
}
...
mod tests {
    ...
    fn config_path_used_when_env_unset() {
        let cfg = TrackConfig {
            path: Some(PathBuf::from("/tmp/sigfold-track-cfg.jsonl")),
            ..TrackConfig::default()
        };
        assert_eq!(
            resolve_track_path_with(&cfg, |_| None),
            Some(PathBuf::from("/tmp/sigfold-track-cfg.jsonl")),
    ...
    fn env_wins_over_config_path() {
        ...
        let path = resolve_track_path_with(&cfg, |key| {
            (key == "SIGFOLD_TRACK_FILE").then(|| "/tmp/sigfold-track-env.jsonl".to_string())
        });
        assert_eq!(
            path,
            Some(PathBuf::from("/tmp/sigfold-track-env.jsonl")),
            "env var wins over config path"
        );
    }

    #[test]
    fn default_ends_in_sigfold_tracking_jsonl() {
        ...
        assert!(path.ends_with("sigfold/tracking.jsonl"));
    }
}
